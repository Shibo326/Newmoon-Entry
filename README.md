...
**Source**: [`contracts/nightscore.compact`](contracts/nightscore.compact) — Compact language (Midnight's ZK smart contract language)

**Compiled Output**: [`managed/`](managed/) — Generated by `compact compile contracts/nightscore.compact managed`
...
**Circuits defined in contract**:
- `computeScore(walletAge, txFrequency, defiInteractions, repaymentHistory, assetDiversity, liquidationHistory)` — Computes weighted credit score from 6 private inputs
- `verifyThreshold(actualScore, minimumScore)` — Proves score meets threshold, disclosing only boolean result

**Deployed Contract Addresses**:

| Network | Contract Address | Status |
|---------|-----------------|--------|
| Local Devnet | `a3e01772c31935fc25719d878514b2bb1b64198c65b4862dd9fcb6888173af71` | Deployed |
| Preprod | *Pending faucet funding* | Ready to deploy |
...
2. Generate/reuse a wallet, print the address and faucet URL
3. Wait for you to fund via faucet (`https://midnight-tmnight-preprod.nethermind.dev`)
4. Register NIGHT for DUST generation
5. Deploy the NightScore contract and save the address
...
- **X (Twitter)**: [@Secret_324](https://x.com/Secret_324)
- **Live Demo**: https://newmoon-entry-projects.vercel.app
- **GitHub**: https://github.com/Shibo326/Newmoon-Entry
...
├── src/
│   ├── deploy.ts         ← Deploy NightScore to public network
│   ├── setup.ts          ← One-shot orchestrator
│   ├── network.ts        ← Network config + state management
│   ├── wallet.ts         ← Wallet SDK wrapper
│   └── check-balance.ts  ← Balance check utility
└── docker-compose.yml    ← Proof server for public networks
